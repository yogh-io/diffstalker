...
- **REST:** `GET /repos/:id/journal?since=<seq>` → `{ epoch, lastSeq, prunedBefore, entries[] }` in a new `packages/daemon/src/routes/journal.ts`. `since` gives cheap catch-up; `prunedBefore` exposes ring-buffer eviction honestly.
