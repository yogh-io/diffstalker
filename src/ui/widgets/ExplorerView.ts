...
// Legacy interface support - convert from old ExplorerItem[] to new format
// This is for backwards compatibility with App.ts which may still pass ExplorerItem[]
export { ExplorerDisplayRow };

// Re-export ExplorerItem for backwards compatibility
...
// Overload to handle both new and old format
export function formatExplorerViewCompat(
  items: ExplorerItem[] | ExplorerDisplayRow[],
  selectedIndex: number,
  isFocused: boolean,
  width: number,
  scrollOffset: number = 0,
  ...
  // Check if items are ExplorerDisplayRow (have 'node' property)
  if (items.length > 0 && 'node' in items[0]) {
    return formatExplorerView(
      items as ExplorerDisplayRow[],
      selectedIndex,
      isFocused,
      width,
      scrollOffset,
  ...
  // Convert old ExplorerItem[] to ExplorerDisplayRow[]
  const displayRows: ExplorerDisplayRow[] = (items as ExplorerItem[]).map((item, i) => ({
    node: {
      name: item.name,
      path: item.path,
