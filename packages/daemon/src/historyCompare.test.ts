...
async function openRepo(repoDir: string): Promise<string> {
...
beforeAll(async () => {
  ...
  repoId = await openRepo(repoPath);
  noBaseRepoId = await openRepo(noBasePath);
});
...
describe('compare endpoints', () => {
  test('GET /compare/base falls back to the discovered default', async () => {
    // Runs before the PUT below: the cache is empty (own XDG_CACHE_HOME).
    const res = await request(`/repos/${repoId}/compare/base`);
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ base: 'origin/main' });
  });
  ...
  test('PUT /compare/base persists and a later GET reflects it', async () => {
    ...
    expect(fs.readFileSync(cacheFile, 'utf-8')).toContain('"main"');
  });
  ...
  test('GET /compare without base resolves the persisted one', async () => {
    // The PUT above set 'main'; omitting base must use it.
  ...
  test('GET /compare with an unknown base ref is a 400', async () => {
  ...
  test('GET /compare with no resolvable base is a 400', async () => {
    const res = await request(`/repos/${noBaseRepoId}/compare`);
    expect(res.status).toBe(400);
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe('no base branch');
  });
});
