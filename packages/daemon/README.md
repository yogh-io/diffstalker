...
| GET    | `/repos/:id/compare/file?path=&base=&uncommitted=&whole=` | One file's diff inside a comparison, as a `DiffResult`. The stacked views pull a range whole and split it client-side, so they have no per-file request of their own to widen. `uncommitted=true` reads the row against HEAD instead of against the base — Compare's stack mixes the two and they are genuinely different comparisons. Same rename-preserving pathspec as above. 400 without `path` |
