/**
 * Theme tables shared by the CLI and the web UI.
 * Browser-safe: pure data and lookups, no node/UI imports.
 *
 * This is the single source for the six theme names, their display names,
 * and the 10-field diff palette. Color values are either hex or named ANSI
 * colors — blessed takes both directly; the web resolves named colors to
 * hex in its palette layer. Package-specific extensions (the web's chrome
 * and syntax tokens) live in each package's own themes module.
 */
